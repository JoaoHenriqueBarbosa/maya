...
func TestContainer_Build(t *testing.T) {
	...
	// A container with an opaque color is decorated, so Build returns a
	// RenderDecoratedBox whose child is the RenderBox holding the layout size.
	decorated, ok := renderObj.(*RenderDecoratedBox)
	if !ok {
		t.Fatalf("Container.Build with a color should return RenderDecoratedBox, got %T", renderObj)
	}
	if _, ok := decorated.Child.(*RenderBox); !ok {
		t.Fatalf("decorated box child should be a RenderBox, got %T", decorated.Child)
	}
}
